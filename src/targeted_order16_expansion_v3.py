#!/usr/bin/env python3
"""Versioned tail expansion for the targeted order-16 bipartite census."""
...
PRIOR_RESULTS = ROOT / "results/targeted-order16/expansion-v3-prior-indices.txt"
OUTPUT_JSON = ROOT / "results/targeted-order16/expansion-v3-summary.json"
CHECKPOINT_DIR = ROOT / "results/targeted-order16/expansion-v3"
CHECKPOINT_JSONL = CHECKPOINT_DIR / "expansion-v3.jsonl"
...
def original_indices() -> set[int]:
    ...
    if PRIOR_RESULTS.exists():
        indices = {
            int(line.strip())
            for line in PRIOR_RESULTS.read_text(encoding="utf-8").splitlines()
            if line.strip()
        }
    return indices
